...
export default function VendaRapidaFormClient({ vendedorOptions, parcelasOptions, onSubmitAction }: Props) {
            ...
            <div className="relative">
              <div className="relative">
                <Input 
                  ...
                  className={`pl-10 h-11 transition-all ${selectedCliente ? "border-emerald-500 bg-emerald-50/30 ring-emerald-500/20" : ""}`}
                  required
                />
                ...
                {selectedCliente && (
                  <div className="absolute right-3 top-1/2 -translate-y-1/2">
                    <Badge variant="outline" className="bg-emerald-100 text-emerald-700 border-emerald-200">Selecionado</Badge>
                  </div>
                )}
              </div>
              
              {clienteSuggestions.length > 0 && (
                <div className="absolute z-[100] w-full mt-1 bg-white border rounded-lg shadow-xl max-h-60 overflow-auto animate-in fade-in zoom-in-95">
                  {clienteSuggestions.map((c) => (
                    <div 
                      key={c.id} 
                      className="p-3 hover:bg-blue-50 cursor-pointer text-sm flex justify-between items-center border-b last:border-0 transition-colors"
                      onClick={() => {
          ...
          <CardContent className="px-4 pb-4">
            <div className="grid gap-4">
              <div className="relative">
                <div className="relative">
                  <Input 
                    ...
                    className={`pl-10 h-11 transition-all ${selectedProduto ? "border-emerald-500 bg-emerald-50/30" : ""}`}
                    required
                  />
                ...
                {produtoSuggestions.length > 0 && (
                  <div className="absolute z-[90] w-full mt-1 bg-white border rounded-lg shadow-xl max-h-60 overflow-auto animate-in fade-in zoom-in-95">
                    {produtoSuggestions.map((p) => (
                      ...
                      >
                        ...
                        <span className="font-bold text-blue-700">{formatBRL(p.valorVenda || 0)}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {aiSuggestions && (aiSuggestions.upsell.length > 0 || aiSuggestions.crossSell.length > 0) && (
                <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border border-purple-100 rounded-xl p-4 mt-2">
                  <div className="flex items-center gap-2 text-purple-700 font-bold text-xs uppercase mb-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {[...aiSuggestions.upsell, ...aiSuggestions.crossSell].slice(0, 2).map((s) => (
                      <div 
                        ...
                        className="flex items-center justify-between p-3 rounded-lg bg-white/80 hover:bg-white border border-purple-200 cursor-pointer transition-all hover:shadow-md group"
                      >
                        <div className="flex flex-col overflow-hidden">
                          <Badge variant="secondary" className="w-fit text-[9px] h-4 mb-1 bg-purple-100 text-purple-700 hover:bg-purple-100">
                            {s.label === 'Upsell' ? 'Opção Premium' : 'Combina com'}
                          </Badge>
                          <span className="text-xs font-bold text-slate-800 truncate group-hover:text-purple-700">{s.nome}</span>
                          <span className="text-[10px] text-muted-foreground font-medium">{formatBRL(s.valorVenda)}</span>
                        </div>
                        <div className="h-8 w-8 rounded-full bg-purple-50 flex items-center justify-center group-hover:bg-purple-600 transition-colors">
                          <ShoppingCart className="h-4 w-4 text-purple-600 group-hover:text-white" />
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <Card className="border-none shadow-sm bg-white">
          ...
          <CardContent className="px-4 pb-4">
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              ...
              <div className="space-y-1">
                <Label className="text-[10px] font-bold uppercase text-muted-foreground">Custo Produto</Label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-xs font-bold">R$</span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
              <div className="space-y-1">
                <Label className="text-[10px] font-bold uppercase text-muted-foreground">Custo Frete</Label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-xs font-bold">R$</span>
              ...
              <div className="col-span-1 md:col-span-2 flex items-end">
                <div className="flex items-center space-x-2 border rounded-md h-10 px-3 w-full bg-slate-50/50">
                  <input 
                    type="checkbox" 
                    id="temNota" 
                    checked={temNota} 
      ...
      <div className="lg:col-span-4 space-y-4">
        <div className="sticky top-6">
          <Card className="border-none shadow-lg bg-blue-900 text-white overflow-hidden">
            <div className="p-5">
              ...
              <Button 
                ...
                type="submit" 
                className="w-full h-14 bg-emerald-500 hover:bg-emerald-600 text-emerald-950 font-black text-lg shadow-[0_4px_20px_rgba(16,185,129,0.3)] transition-all active:scale-[0.98]"
              >
          ...
          <div className="mt-4 space-y-4">
            {limiteData && (
              <Card className={`border-2 transition-colors ${showCreditWarning ? 'bg-amber-50 border-amber-200' : 'bg-emerald-50 border-emerald-200'}`}>
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {showCreditWarning && (
                      <div className="flex items-start gap-2 text-[10px] text-amber-800 bg-amber-100/50 p-2 rounded border border-amber-200 mt-2">
            ...
            {!selectedCliente && (
              <div className="p-4 rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 flex flex-col items-center justify-center text-center space-y-2 opacity-60">
